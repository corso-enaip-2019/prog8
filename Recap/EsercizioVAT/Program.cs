...
namespace EsercizioVAT
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bool operationChosenBool = int.TryParse(Console.ReadLine(), out int operationChosen);
            Console.WriteLine($"hai scelto l'opzione {operationChosen}");
            
            if(operationChosen == 1)
            {
                Console.Write("Inserire il numero di partita IVA su cui eseguire l'operazione: ");
                bool vatNumberBool = int.TryParse(Console.ReadLine(), out int vatNumberInput);
